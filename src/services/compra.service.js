...
import { generarIdLote } from "../utils/stockOperations.js";

export const crearCompras = async (comprasInput, userId, sede) => {
...
export const actualizarLoteCompras = async (ids, nuevasCompras, userId, sede) => {
  ...
  for (const compra of comprasPrevias) {
    ...
    if (producto) {
      ...
      if (nuevoIngreso < 0) {
        throw new Error(`No se puede actualizar: el producto "${producto.nombre}" quedaría con ingresos negativos.`);
      }
  ...
  for (const compra of nuevasCompras) {
    ...
    if (typeof cantidad !== "number" || cantidad < 0 ||
      ...
      throw new Error(`Datos inválidos en la nueva compra: cantidad y precio_compra deben ser números >= 0`);
    }
...
export const eliminarCompraPorId = async (compraId) => {
  ...
  if (producto) {
    ...
    if (producto.ingresos - resta < 0) {
      throw new Error("No se puede eliminar: generaría un valor negativo en ingresos del producto.");
    }
...
export const eliminarLoteComprasPorId = async (id_lote, sede) => {
  const compras = await Compra.find({ id_lote, sede });
  if (!compras.length) throw new Error("Lote no encontrado");

  for (const compra of compras) {
    const usado = compra.cantidad - compra.cantidad_disponible;
    if (usado > 0) {
      throw new Error(`No se puede eliminar el lote ${id_lote} porque ya ha sido parcialmente utilizado.`);
    }
    ...
    if (producto) {
      const resta = compra.cantidad_disponible;
      if (producto.ingresos - resta < 0) {
        throw new Error(`Eliminar el lote ${id_lote} generaría un valor negativo en ingresos del producto.`);
      }
      producto.ingresos -= resta;
      await producto.save();
    }
...
export const actualizarCompraIndividual = async (compraId, nuevosDatos) => {
  ...
  if (nuevaCantidad < 0 || nuevoPrecio < 0) {
    throw new Error("Cantidad y precio de compra deben ser mayores o iguales a 0");
  }

  const cantidadUsada = compra.cantidad - compra.cantidad_disponible;
  if (nuevaCantidad < cantidadUsada) {
    throw new Error("No se puede reducir la cantidad por debajo de la cantidad ya usada.");
  }

  const diferencia = nuevaCantidad - compra.cantidad;

  const productoAnterior = await Product.findById(compra.producto);
  if (!productoAnterior) throw new Error("Producto original de la compra no encontrado.");

  const productoNuevo = nuevosDatos.producto
    ? await Product.findById(nuevosDatos.producto)
    : productoAnterior;
  if (!productoNuevo) throw new Error("Nuevo producto no encontrado.");

  if (productoAnterior._id.toString() === productoNuevo._id.toString()) {
    if (productoAnterior.ingresos + diferencia < 0) {
      throw new Error("La actualización haría que los ingresos del producto sean negativos.");
    }
    productoAnterior.ingresos += diferencia;
    await productoAnterior.save();
  } else {
    if (productoAnterior.ingresos - compra.cantidad < 0) {
      throw new Error("No se puede trasladar esta compra: afectaría los ingresos originales.");
    }
