...
export const registrarReserva = async (req, res) => {
  try {
    ...
    const resultado = await crearReserva(req.body, req.sede);
...
export const obtenerReservas = async (req, res) => {
  try {
    ...
    const reservas = await getReservas(opts, req.sede);
...
export const obtenerReservaPorId = async (req, res) => {
  try {
    const { id } = req.params;
    const resultado = await getReservaById(id, req.sede);
...
export const editarReserva = async (req, res) => {
  try {
    ...
    const resultado = await actualizarReserva(id, req.body, req.sede);
    res.status(200).json(resultado);
  } catch (error) {
    res.status(400).json({ mensaje: error.message });
  }
...
export const cancelarReserva = async (req, res) => {
  try {
    ...
    const resultado = await eliminarReserva(id, usuarioId, motivo, req.sede);
    res.status(200).json(resultado);
  } catch (error) {
    res.status(400).json({ mensaje: error.message });
  }
